...
/// <summary>魂灯次级资源的独立战斗计数器。</summary>
[GlobalClass]
public partial class NQueenSoulLampCounter : Control
{
	...
	public override void _Ready()
	{
		...
		Refresh(_boundPlayer, false);
	}

	public void Refresh(Player? player, IReadOnlyList<SecondaryResourceDefinition> visibleDefinitions)
	{
		bool forceVisible = IsSoulLampVisibleInCombatUi(visibleDefinitions);
		Refresh(player, forceVisible);
	}

	public void Refresh(Player? player, bool forceVisible)
	{
		if (!ReferenceEquals(_boundPlayer, player))
		{
			_boundPlayer = player;
			_displayedSoulLampAmount = int.MinValue;
		...
		_hasBeenMaterial = _hasBeenMaterial || amount > SoulLampResources.SoulLampDefinition.DefaultAmount;
		Visible = _hasBeenMaterial || forceVisible;
		ApplySoulLampDisplay(player, amount);
	}

	private static bool IsSoulLampVisibleInCombatUi(IReadOnlyList<SecondaryResourceDefinition> visibleDefinitions)
	{
		for (int i = 0; i < visibleDefinitions.Count; i++)
		{
			if (string.Equals(
				    visibleDefinitions[i].Id,
